...
export default function LiveCard() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setPrice((p) => {
        ...
        setPts((prev) => { const a = [...prev]; a.push(120 + (next - 142.5)); a.shift(); return a })
        return next
      })
      setFlash(true)
  ...
  function fitPoints(arr: number[]) {
    if (arr.length < 2) return '0,27 300,27'
  ...
  if (!mounted) {
    return (
      <div className="rounded-[20px] p-5 h-[280px]" style={{ background: 'linear-gradient(180deg, #151B30, #0F1424)', border: '1px solid rgba(244,233,107,0.10)' }} />
    )
  }
  ...
  return (
    <div className="relative rounded-[20px] p-5 overflow-hidden" style={{
      ...
      <div className="relative">
        <div className="text-[10.5px] tracking-[0.2em] uppercase text-[#8A93B0]" style={{ fontFamily: 'var(--font-mono-about, monospace)' }}>Live price</div>
        ...
        <div className="flex items-center gap-3 my-3.5">
          <div className="w-[46px] h-[46px] rounded-[14px] flex items-center justify-center text-[#0A0D18] text-xl font-bold flex-shrink-0"
            style={{ background: 'linear-gradient(135deg, #F4E96B, #B98E2E)', fontFamily: 'var(--font-display, system-ui)' }}>
            M
          </div>
          <div>
            <div className="font-semibold text-[17px]" style={{ fontFamily: 'var(--font-display, system-ui)' }}>@MidnightPlays</div>
        ...
        <div className="flex justify-between items-baseline text-[13px] text-[#8A93B0] border-t border-dashed border-white/[0.07] pt-3"
          style={{ fontFamily: 'var(--font-mono-about, monospace)' }}>
        ...
        <div className="flex items-end justify-between mt-1.5">
          <div>
            <div className="text-[11px] tracking-[0.18em] uppercase text-[#8A93B0]" style={{ fontFamily: 'var(--font-mono-about, monospace)' }}>Price</div>
            <div className="font-bold text-[34px] leading-none tracking-tight" style={{ fontFamily: 'var(--font-mono-about, monospace)' }}>
              {price.toFixed(2)}
            </div>
          </div>
          <div className={`font-bold text-sm px-2.5 py-1 rounded-lg transition-colors`}
            style={{
              fontFamily: 'var(--font-mono-about, monospace)',
              color: isUp ? '#4ADE80' : '#F87171',
              background: flash
                ? (isUp ? 'rgba(74,222,128,0.34)' : 'rgba(248,113,113,0.34)')
                : (isUp ? 'rgba(74,222,128,0.10)' : 'rgba(248,113,113,0.10)'),
            }}>
