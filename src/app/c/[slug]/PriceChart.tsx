...
export default function PriceChart({ history, currentPrice, initialPrice }: Props) {
  ...
  const isUp = currentPrice >= initialPrice, stroke = isUp ? '#22C55E' : '#EF4444'
    ...
    <div className="h-[220px] w-full select-none">
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={d} margin={{ top: 8, right: 8, bottom: 4, left: 0 }}>
          ...
          <YAxis domain={[ticks[0], ticks[ticks.length - 1]]} ticks={ticks} axisLine={false} tickLine={false} tick={{ fill: '#8A8A82', fontSize: 11, fontFamily: 'SF Mono, Menlo, monospace' }} tickFormatter={(v: number) => `$${v % 1 === 0 ? v : v.toFixed(2)}`} width={52} />
          <Tooltip cursor={{ stroke: '#2A2A2E', strokeWidth: 1 }} contentStyle={{ background: '#16161A', border: '1px solid #2A2A2E', borderRadius: '10px', padding: '8px 12px', boxShadow: '0 8px 32px rgba(0,0,0,0.6)' }} formatter={(value: number) => [`$${value.toFixed(2)}`, 'Price']} labelFormatter={(i: number) => i === 0 ? 'Launch' : `Trade ${i}`} labelStyle={{ color: '#8A8A82', fontSize: 11, marginBottom: 2 }} itemStyle={{ color: '#F5F5F0', fontWeight: 600, fontSize: 14 }} />
          <Area type="monotone" dataKey="price" stroke={stroke} strokeWidth={2} fill="url(#cg)" dot={false} activeDot={{ r: 4, fill: stroke, stroke: '#16161A', strokeWidth: 2 }} animationDuration={600} animationEasing="ease-out" />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  )
