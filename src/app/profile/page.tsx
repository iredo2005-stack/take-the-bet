  ...
  return (
    <div className="min-h-screen bg-bg pb-24">
      ...
      <main className="max-w-lg mx-auto px-4 sm:px-6 py-8">
        <h1 className="text-2xl font-bold text-[#1E2329] mb-6">Your Profile</h1>
        ...
        <div className="bg-card border border-edge rounded-2xl p-5 mb-4 flex items-center gap-4">
          ...
          <div>
            <p className="text-[#1E2329] font-semibold text-lg">{user.full_name || 'Anonymous'}</p>
            <p className="text-[#707A8A] text-sm">{user.email}</p>
            <p className="text-[#707A8A] text-xs mt-1">Member since {new Date(user.created_at).toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}</p>
          </div>
        </div>
        ...
        <div className="space-y-2 mt-6">
          {user.username && (
            <Link href={`/trader/${user.username}`} className="block bg-card border border-edge rounded-xl p-4 hover:border-accent/30 transition-colors">
              <div className="flex items-center justify-between">
                <div><p className="text-[#1E2329] font-medium text-sm">Public Profile</p><p className="text-[#707A8A] text-xs">View how others see you</p></div>
          ...
          <Link href="/dashboard" className="block bg-card border border-edge rounded-xl p-4 hover:border-accent/30 transition-colors">
            <div className="flex items-center justify-between">
              <div><p className="text-[#1E2329] font-medium text-sm">Dashboard & Portfolio</p><p className="text-[#707A8A] text-xs">View your holdings and balance</p></div>
              <span className="text-accent text-sm">→</span>
            </div>
          </Link>
          <Link href="/leaderboard" className="block bg-card border border-edge rounded-xl p-4 hover:border-accent/30 transition-colors">
            <div className="flex items-center justify-between">
              <div><p className="text-[#1E2329] font-medium text-sm">Leaderboard</p><p className="text-[#707A8A] text-xs">See where you rank</p></div>
              <span className="text-accent text-sm">→</span>
            </div>
          </Link>
        </div>
        ...
        <div className="mt-8 text-center">
          <p className="text-[#707A8A] text-xs">Role: {user.role} · Age verified: {user.age_verified ? '✓' : '✗'}</p>
        </div>
      </main>
    </div>
  )
